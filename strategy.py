# -*- coding: utf-8 -*-
# strategy.py - Spot-only (v4.0 — نسخة محسّنة)
# التغييرات الرئيسية:
# 1. حذف fetch_symbol_filters المحلية (تُستخدم okx_api مباشرة)
# 2. نظام سكور محسّن (عتبة 50، وزن أعلى للعوامل الحقيقية)
# 3. Break-even بعد TP1 (حماية الأرباح)
# 4. Early Scout مشدّد (سوق صاعد فقط + breadth >= 0.55)
# 5. فلتر الاستنزاف (RSI > 75 أو بعد > 8% عن EMA50)
# 6. _mark_signal_now موحّدة (نسخة واحدة فقط)
# 7. Trailing يبدأ بعد TP1 مباشرة
...
from okx_api import (
    ...
    place_market_order,
    fetch_balance,
    fetch_symbol_filters,   # ← من okx_api مباشرة (لا نعرّفها محلياً)
)
...
EARLY_SCOUT_SCORE_MIN    = _env_int("EARLY_SCOUT_SCORE_MIN",   30)   # رُفع من 25
EARLY_SCOUT_MAX_ATR_DIST = _env_float("EARLY_SCOUT_MAX_ATR_DIST", 0.5)  # خُفّض من 0.6
EARLY_SCOUT_BR_MIN       = _env_float("EARLY_SCOUT_BR_MIN", 0.55)   # ← جديد: breadth أدنى
...
# ===== Scoring — عتبة مرفوعة =====
# القيمة الافتراضية 50 (بدل 35)
# يمكن التحكم بها من ENV أو config.py
SCORE_THRESHOLD = _env_int("SCORE_THRESHOLD", 50)

# ===== Exhaustion filter (جديد) =====
EXH_RSI_MAX        = _env_float("EXH_RSI_MAX",        75.0)   # خُفّض من 76
EXH_EMA50_DIST_PCT = _env_float("EXH_EMA50_DIST_PCT",  0.08)  # جديد: 8% بدل ATR مضاعف
EXH_EMA50_DIST_ATR = _env_float("EXH_EMA50_DIST_ATR",  2.8)

# ===== Break-even (جديد) =====
BREAKEVEN_ENABLE      = _env_bool("BREAKEVEN_ENABLE", True)
BREAKEVEN_TRIGGER_ATR = _env_float("BREAKEVEN_TRIGGER_ATR", 0.3)  # بعد 0.3 ATR ربح
BREAKEVEN_OFFSET_PCT  = _env_float("BREAKEVEN_OFFSET_PCT",  0.002) # +0.2% فوق الدخول

# ===== Aggressive Mode =====
AGGR_MODE_ENABLE     = _env_bool("AGGR_MODE_ENABLE",    True)
AGGR_SCORE_MIN       = _env_int("AGGR_SCORE_MIN",        65)   # رُفع من 60
AGGR_SCORE_STRONG    = _env_int("AGGR_SCORE_STRONG",     75)   # رُفع من 70
...
# ===== Breadth =====
BREADTH_MIN_RATIO   = _env_float("BREADTH_MIN_RATIO", 0.55)  # رُفع من 0.60→0.55 (أكثر واقعية)
...
def _opportunity_score(df, prev, closed):
    """
    نظام سكور محسّن v2:
    - وزن أعلى للعوامل الحقيقية
    - حذف النقاط السهلة (BullishClose وحده)
    - العتبة: 50 (من config/ENV)
    - أقصى سكور ممكن: ~100
    """
    score, why, pattern = 0, [], ""
    try:
...
# ================== [UNIFIED] _mark_signal_now ==================
# نسخة واحدة فقط — تكتب في _LAST_ENTRY_BAR_TS
...
def _entry_pullback_logic(df, closed, prev, atr_ltf, htf_ctx, cfg):
    try:
        ...
        # Fail-soft: دخول مبكر إذا الهيكل قوي
        try:
...
def check_signal(symbol: str):
    ...
    try:
        ...
        # ── Early Scout (مشدّد) ──
        # [CHANGED]: trend == "up" فقط (بدل "up" أو "neutral")
        # [CHANGED]: br >= EARLY_SCOUT_BR_MIN (breadth كافٍ)
        if EARLY_SCOUT_ENABLE:
            try:
                ...
                if ema50_val and atr_val and atr_val > 0:
                    ...
                    early_ok = (
                        ema100_trend == "up" and
                        trend == "up" and                         # ← مشدّد: up فقط
                        br is not None and br >= EARLY_SCOUT_BR_MIN and  # ← جديد
...
def execute_buy(symbol, sig=None):
    ...
    pos = {
        ...
        "breakeven_done":False,  # ← جديد: تتبع Break-even
    }
...
def _price_now_safe(sym):
    try:    px = float(fetch_price(sym) or 0.0)
    except: px = 0.0
    if px <= 0.0:
        try:
...
def _safe_sell(base_symbol, want_qty):
    SELL_DUST_MIN  = float(os.getenv("SELL_DUST_MIN_USDT","5.0"))
    SELL_WARN_TTL  = int(os.getenv("SELL_WARN_TTL","1800"))

    try:    avail = float(fetch_balance(base_symbol.split("/")[0]) or 0.0)
    except: avail = 0.0
    if avail <= 0.0:
        _tg_once(f"warn_insuff_{base_symbol}",f"⚠️ لا كمية متاحة لـ {base_symbol}.",ttl_sec=SELL_WARN_TTL)
    ...
    if price_now <= 0.0:
        _tg_once(f"sell_skip_noprice_{base_symbol}",f"⚠️ تخطّي البيع {base_symbol}: السعر غير متاح.",ttl_sec=SELL_WARN_TTL)
        return None, None, 0.0
    ...
    if qty <= 0.0 or (qty*price_now) < SELL_DUST_MIN:
        _tg_once(f"sell_dust_{base_symbol}",f"⚠️ غبار {base_symbol}: {qty*price_now:.2f}$",ttl_sec=SELL_WARN_TTL)
        return None, None, 0.0

    if (qty*price_now) < min_notional:
        need_amt = math.ceil((min_notional/price_now)/step)*step
        if need_amt <= avail: qty = need_amt
        else:
            _tg_once(f"sell_skip_small_{base_symbol}",f"⚠️ قيمة صغيرة {base_symbol}.",ttl_sec=SELL_WARN_TTL)
            return None, None, 0.0
    ...
    try:
        ...
        if "51008" in msg or "insufficient" in msg.lower():
            ...
            if qty2 > 0 and qty2*price_now >= min_notional:
                try:
                    order = place_market_order(base_symbol,"sell",qty2)
                    qty   = qty2
                except:
                    _tg_once(f"sell_fail_{base_symbol}",f"❌ فشل بيع {base_symbol} (51008).",ttl_sec=SELL_WARN_TTL)
                    return None, None, 0.0
            else:
                _tg_once(f"sell_fail_{base_symbol}",f"❌ فشل بيع {base_symbol} (51008).",ttl_sec=SELL_WARN_TTL)
                return None, None, 0.0
        else:
            _tg_once(f"sell_fail_{base_symbol}",f"❌ فشل بيع {base_symbol}: {e}",ttl_sec=SELL_WARN_TTL)
            return None, None, 0.0

    if not order:
        _tg_once(f"sell_fail_{base_symbol}",f"❌ فشل بيع {base_symbol}.",ttl_sec=SELL_WARN_TTL)
        return None, None, 0.0
...
# ================== manage_position (محسّن بـ Break-even) ==================
def manage_position(symbol):
    ...
    # ── [NEW] Break-even: بعد 0.3 ATR ربح ──
        ...
        for i, tp in enumerate(targets):
            if not pos["tp_hits"][i] and current >= float(tp) and pos["amount"] > 0:
                ...
                if order and sold_qty > 0:
                    ...
                    # ── [IMPROVED] قفل بعد TP1 مباشرة (Break-even إضافي) ──
                    ...
                    # ── Trailing بعد TP1 (بدل TP2) ──
                    if i >= 0 and pos["amount"] > 0:  # ← تغيير: i>=0 بدل i>=1
